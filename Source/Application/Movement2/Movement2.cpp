...
void Movement2::InitializeApplication()
{
	...
	for (int i = 0; i < 20; i++)
	{
		...
		auto model = go->AddComponent<AnimatedModel>(AssetManager::Get().GetAsset<MeshAsset>("SK_C_TGA_Bro.fbx")->mesh, AssetManager::Get().GetAsset<MaterialAsset>("TgaBroBlue.json")->material);	model->AddAnimationToLayer("Idle", AssetManager::Get().GetAsset<AnimationAsset>("A_C_TGA_Bro_Idle_Breathing.fbx")->animation, "", true);
