...
float3 Diffuse_IBL(float3 aPixelNormal, TextureCube envCubemap)
{
    ...
    const float3 iblDiffuse = envCubemap.SampleLevel(DefaultSampler, aPixelNormal, numMips).rgb;
    return iblDiffuse;
}
...
float3 Specular_IBL(float3 aPixelNormal, float3 toViewDir, float aRoughness, float3 aSpecularColor, TextureCube envCubemap)
{
    ...
    float3 envColor = envCubemap.SampleLevel(DefaultSampler, R, aRoughness * numMips).rgb;
...
float ShadowFactorDLightSpotLight(float4x4 lightView, float4x4 lightProj, float3 lightDir, float4 worldPos, float3 pixelNormal, float minBias, float maxBias, Texture2D shadowMap, int samples)
{
...
float ShadowFactorPointLight(PointLightData pointLight, float4 worldPos, float3 pixelNormal, float minBias, float maxBias, TextureCube shadowCubemap, int samples)
{
...
float3 BRDF_DirectionalLight(DirLightData dirLight, float3 viewPos, float4 worldPos, float3 pixelNormal, float3 diffuseColor, float3 specularColor, float roughness, Texture2D shadowMap, int shadowSamples)
{
    ...
    float shadowFactor = ShadowFactorDLightSpotLight(dirLight.View, dirLight.Projection, dirLight.Direction.xyz, worldPos, pixelNormal, 0.0001, 0.0005, shadowMap, shadowSamples);
...
float3 BRDF_PointLight(PointLightData pointLight, float3 viewPos, float4 worldPos, float3 pixelNormal, float3 diffuseColor, float3 specularColor, float roughness, TextureCube shadowCubemap, int shadowSamples)
{
    ...
    float shadowFactor = ShadowFactorPointLight(pointLight, worldPos, pixelNormal, 0.0001, 0.0003, shadowCubemap, shadowSamples);
    directLight *= saturate(shadowFactor);
    return directLight;
}
...
float3 BRDF_SpotLight(SpotLightData spotLight, float3 viewPos, float4 worldPos, float3 pixelNormal, float3 diffuseColor, float3 specularColor, float roughness, Texture2D shadowMap, int shadowSamples)
{
    ...
    float shadowFactor = ShadowFactorDLightSpotLight(spotLight.View, spotLight.Projection, spotLight.Direction, worldPos, pixelNormal, 0.0001, 0.0003, shadowMap, shadowSamples);
    directLight *= saturate(shadowFactor);
    return directLight;
}
